...
/**
 * {@code config/slashlootr.json}. Band A has no loader bridge, so the directory comes straight
 * from FabricLoader.
 *
 * <p>Blocklists are compared as plain namespaced strings, matching the shared tree so the same
 * config file works on every band.
 *
...
public final class SlashLootrConfig {
    ...
    public static void load() {
        path = FabricLoader.getInstance().getConfigDir().resolve("slashlootr.json");
        reload();
    }
